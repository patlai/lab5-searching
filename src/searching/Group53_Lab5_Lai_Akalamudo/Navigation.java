...
public class Navigation {
	final static int SUPER_FAST = 300, FAST = 200, SLOW = 100, ACCELERATION = 4000;
	final static double DEG_ERR = 5.0, CM_ERR = 3.0;
	...
	public void travelTo(double x, double y) {
		...
		while (Math.abs(x - odometer.getX()) > CM_ERR || Math.abs(y - odometer.getY()) > CM_ERR) {
			if(USLocalizer.isCloseToObject){
				//				leftMotor.rotate(convertDistance(Lab5.WHEEL_RADIUS, USLocalizer.OBJECT_DETECTION_THRESHOLD - 5));
				//				rightMotor.rotate(convertDistance(Lab5.WHEEL_RADIUS, USLocalizer.OBJECT_DETECTION_THRESHOLD - 5));
				//				Delay.msDelay(1000);
				break;
			}
...
//			rightMotor.rotate(360,true);
	...
	public void scanForObject(){
		...
		Delay.msDelay(200);
		leftMotor.rotate(Navigation.convertAngle(Lab5.WHEEL_RADIUS, Lab5.TRACK, 90), true);
		rightMotor.rotate(Navigation.convertAngle(Lab5.WHEEL_RADIUS, Lab5.TRACK, 9-0), false);
	}
}
